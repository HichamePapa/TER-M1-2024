...
eraseComplete(D):- askErase(D,T),used(P,D,_R,TU),action(P,'delete'),((T-TU)<TLIMITERASE).
eraseNotDoneYet(D):- askErase(D,T),\+ (used(P,D,_R,_TU),action(P,'delete')),((TCURRENT-T)<TLIMITERASE).
