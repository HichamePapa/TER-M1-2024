writeStorageLimitationNotCompliant(D,TU):- write('STORAGE LIMITATION ISSUE - data '), write(D), write(' was last used at '), write(TU), writeln(' and not deleted in time'), false.
